...
type Favorite struct {
	ID            int            `json:"id"`             // id
	FavorableType sql.NullString `json:"favorable_type"` // favorable_type
	FavorableID   sql.NullInt64  `json:"favorable_id"`   // favorable_id
...
func (f *Favorite) Insert(db XODB) error {
	...
	const sqlstr = `INSERT INTO news.favorites (` +
		`favorable_type, favorable_id, user_id, created_at` +
		`) VALUES (` +
		`?, ?, ?, ?` +
		`)`

	// run query
	XOLog(sqlstr, f.FavorableType, f.FavorableID, f.UserID, f.CreatedAt)
	res, err := db.Exec(sqlstr, f.FavorableType, f.FavorableID, f.UserID, f.CreatedAt)
	if err != nil {
		return err
	}
...
func (f *Favorite) Update(db XODB) error {
	...
	// run query
	XOLog(sqlstr, f.FavorableType, f.FavorableID, f.UserID, f.CreatedAt, f.ID)
	_, err = db.Exec(sqlstr, f.FavorableType, f.FavorableID, f.UserID, f.CreatedAt, f.ID)
	return err
}
...
func FavoriteByID(db XODB, id int) (*Favorite, error) {
	...
	// sql query
	const sqlstr = `SELECT ` +
		`id, favorable_type, favorable_id, user_id, created_at ` +
		`FROM news.favorites ` +
		`WHERE id = ?`
	...
	err = db.QueryRow(sqlstr, id).Scan(&f.ID, &f.FavorableType, &f.FavorableID, &f.UserID, &f.CreatedAt)
	if err != nil {
		return nil, err
	}
