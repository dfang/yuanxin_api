...
type Captcha struct {
	...
	CaptchaCode sql.NullString `json:"captcha_code"` // captcha_code
...
func (c *Captcha) Insert(db XODB) error {
	...
	const sqlstr = `INSERT INTO news.captcha (` +
		`user_id, captcha_code` +
		`) VALUES (` +
		`?, ?` +
		`)`

	// run query
	XOLog(sqlstr, c.UserID, c.CaptchaCode)
	res, err := db.Exec(sqlstr, c.UserID, c.CaptchaCode)
	if err != nil {
		return err
	}
...
func (c *Captcha) Update(db XODB) error {
	...
	// run query
	XOLog(sqlstr, c.UserID, c.CaptchaCode, c.ID)
	_, err = db.Exec(sqlstr, c.UserID, c.CaptchaCode, c.ID)
	return err
}
...
func CaptchaByID(db XODB, id int) (*Captcha, error) {
	...
	err = db.QueryRow(sqlstr, id).Scan(&c.ID, &c.UserID, &c.CaptchaCode)
	if err != nil {
		return nil, err
	}
